...
class Expression {
    ...
    init(model, op, properties, params) {
        ...
        if (!this.table.client) {
            throw new Error_1.OneArgError('Table has not yet defined a "client" instance');
        }
    }
    prepare() {
        ...
        if (this.mapped) {
            for (let [att, props] of Object.entries(this.mapped)) {
                if (Object.keys(props).length != this.model.mappings[att].length) {
                    throw new Error_1.OneArgError(`Missing properties for mapped data field "${att}" in model "${this.model.name}"`);
                }
            }
        ...
        if (op == 'find') {
            ...
            if (att == this.sort && typeof value == 'object' && Object.keys(value).length > 0) {
                ...
                if (KeyOperators.indexOf(action) < 0) {
                    throw new Error_1.OneArgError(`Invalid KeyCondition operator "${action}"`);
                }
    ...
    addUpdates() {
        ...
        if (params.add) {
            for (let [key, value] of Object.entries(params.add)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_1.OneArgError('Cannot add to hash or sort');
                }
        ...
        if (params.delete) {
            for (let [key, value] of Object.entries(params.delete)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_1.OneArgError('Cannot delete hash or sort');
                }
                this.already[key] = true;
                let target = this.makeTarget(fields, key);
        ...
        if (params.remove) {
            ...
            for (let key of params.remove) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_1.OneArgError('Cannot remove hash or sort');
                }
                this.already[key] = true;
                let target = this.makeTarget(fields, key);
        ...
        if (params.set) {
            for (let [key, value] of Object.entries(params.set)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_1.OneArgError('Cannot set hash or sort');
                }
                this.already[key] = true;
                let target = this.makeTarget(fields, key);
    ...
    command() {
        ...
        if (key == null && values[hash] == null && op != 'scan') {
            throw new Error_1.OneError(`Cannot find hash key for "${op}"`, { values });
        }
        ...
        if (params.batch) {
            if (op == 'get') {
                ...
                throw new Error_1.OneArgError(`Unsupported batch operation "${op}"`);
            }
            if (filters.length) {
                throw new Error_1.OneArgError('Invalid filters with batch operation');
            }
        }
        else {
            args = {
            ...
            if (params.select) {
                ...
                if (project.length && params.select != 'SPECIFIC_ATTRIBUTES') {
                    throw new Error_1.OneArgError('Select must be SPECIFIC_ATTRIBUTES with projection expressions');
                }
                args.Select = params.select;
            }
            else if (params.count) {
                if (project.length) {
                    throw new Error_1.OneArgError('Cannot use select and count together');
                }
                args.Select = 'COUNT';
            }
